...
**Implementation note (updated UX direction):** simplify apparel sizing in the app. For tops / outerwear / tailoring, normalize to a single clothing-letter system and display it as **Top Size**. For bottoms, keep waist sizing and display it as **Bottom Size**. For shoes and accessories, the field label can stay generic as **Size**. Internal keys may remain unchanged; this is a UX/display simplification.
...
- `EU_CLOTHING`: 44, 46, 48, 50, 52, 54, 56 (internal translation source only for apparel normalization)
- `IT_CLOTHING`: 44, 46, 48, 50, 52, 54, 56 (internal translation source only for apparel normalization)
- `ONE_SIZE`: ONE SIZE

Category group to valid size systems:

- footwear → US_MENS_SHOE, US_WOMENS_SHOE, EU_SHOE, UK_SHOE
- tops, outerwear, tailoring → app UX exposes only CLOTHING_LETTER (displayed as Top Size); AI/server may translate EU_CLOTHING / IT_CLOTHING inputs into that scale
- bottoms → PANTS_WAIST (displayed as Bottom Size); do not collapse bottoms into clothing-letter sizing
...
- Replace size `TextInput` (~line 416) with structured size editing:
  1. For apparel tops/outerwear/tailoring, show a single **Top Size** control using the clothing-letter scale
  2. For bottoms, show **Bottom Size** using waist sizes
  3. For shoes/accessories, label the field generically as **Size**
...
- Apparel translation should happen before UX display/save so users see normalized Top Size values (for example EU 48 → M)
- Use this approved top-size conversion table:
  - IT 44/46/48/50/52/54/56 → XS/S/M/L/XL/XXL/3XL
  - EU 44/46/48/50/52/54/56 → XS/S/M/L/XL/XXL/3XL
  - US 34/36/38/40/42/44/46 → XS/S/M/L/XL/XXL/3XL
  - JP 1/2/3/4/5/6 → S/M/L/XL/XXL/3XL
- `XXXS` and `XXS` are out of scope and should not be offered in the app UX or generated by normalization
